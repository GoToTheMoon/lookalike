...
/**
  * Created by jiaxin.zhu on 2017/5/24.
  */
object deviceTagToKafka extends Serializable {
  ...
  def collectAsBatch(arrBuffer: ArrayBuffer[ArrayBuffer[KeyedMessage[String, String]]], message: String, topic: String, batchSize: Int) = {
    ...
    if (arrBuffer.last.length < batchSize) {
      arrBuffer.last += keyedMessage;
      arrBuffer;
    }
    else arrBuffer += {
      new ArrayBuffer[KeyedMessage[String, String]](batchSize) += keyedMessage
    }
  }
  ...
  def toKafkaArray(data: Array[(String, String)]): Unit = {
    val inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    val time = inputFormat.format(new Date())
    //暂时用当前时间
    val producer = createKafkaProducer()
    data.foldLeft(ArrayBuffer(new ArrayBuffer[KeyedMessage[String, String]](KAFKA_BATCH_SIZE)))((x, y) => {
      val (uid, tag) = y;
      val tagField = tag.replace(",", "!") + s"|~|$time"
      // 实时标签的tag中的","必须转换成"!"
      val message = Array(channelId, "", uid, tagField).mkString("\t")
